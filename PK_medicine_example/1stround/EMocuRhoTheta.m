function [emocu_rho, emocu_theta] = EMocuRhoTheta(theta_hyperpara, rho_hyperpara, r, c, blurringT, sigma_)
%Author: Guang Zhao
%Date:   Dec. 2018
...
%Monta Carlo
N = 400;
rho_samples = 2*betarnd(rho_hyperpara, rho_hyperpara, 1, N)-1;
theta_samples = rand(1, N)*theta_hyperpara-0.5*theta_hyperpara+5;% theta is uniform distribution or Gaussian distribution or...
ocu_rho_array = zeros(N, 1);
ocu_theta_array = zeros(N, 1);
filename = sprintf('Thetaf%.1fRho%.1f.txt', theta_hyperpara, rho_hyperpara);
...
%     IBR_filter_rho = IbrFilterRho(theta_cell, Rho, sigma_);
    ...
    [ocu_rho_array(n), ocu_theta_array(n)] = OcuRhoTheta(theta_samples(n), rho_samples(n),...
        rho_hyperpara, theta_hyperpara,  r, c, blurringT, sigma_);
